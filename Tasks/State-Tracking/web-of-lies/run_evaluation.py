#!/usr/bin/env python3
"""
Evaluate a Causal LM on the BBH web_of_lies task.
...
from web_of_lies_eval import (  # noqa: E402
    accuracy_by_answer,
    accuracy_by_chain_depth,
    evaluate_subset,
    load_task_examples,
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        help="Directory to cache the BBH dataset download.",
    )
    p.add_argument(
        "--output_json",
        type=str,
        default=None,
        help="Output JSON path. Only valid for a single configuration.",
    )
    return p.parse_args()
...
def run_single_config(
    *,
    ...
    indices, pool_sizes = stratified_indices_by_answer(
        examples, rng, n_per_bin=n_per_bin
    )
    ...
    by_depth = accuracy_by_chain_depth(results["rows"])
    ...
    for d in available_depths:
        b = by_depth[d]
        print(f"  chain_depth={d}: n={b['n']} accuracy={b['accuracy']:.4f}")
    ...
    res_to_save = {
        ...
        "available_chain_depths": available_depths,
        "overall": {
            "n": results["n"],
            "accuracy": results["accuracy"],
        },
        "by_answer": {k: v for k, v in by_ans.items()},
        "by_chain_depth": {str(k): v for k, v in by_depth.items()},
        "rows": results["rows"],
    }
...
def main() -> None:
    ...
    print(f"Loaded {len(examples)} examples from BBH web_of_lies.")
    ...
    if failures:
        ...
        for prompt_mode, ablate_group, error in failures:
            print(f"  prompt_mode={prompt_mode}, ablate_group={ablate_group}: {error}")
        sys.exit(1)
