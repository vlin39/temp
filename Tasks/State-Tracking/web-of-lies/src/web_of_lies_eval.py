"""
Web-of-lies eval for raw and chat-style prompting.

Loads the BBH web_of_lies task via `datasets.load_dataset("lukaemon/bbh", "web_of_lies")`.
Each example is a chain of meta-statements; the model must answer "yes" or "no".
"""
...
def _count_chain_depth(input_text: str) -> int:
    """Count the number of truth/lie statements in the chain."""
    matches = re.findall(
...
def load_task_examples(cache_dir: Path | None = None) -> list[dict]:
    """Load BBH web_of_lies from HuggingFace and return a list of dicts.

    Each dict has keys: 'input', 'target', 'gold_answer' (yes/no lower-cased),
    'chain_depth' (number of truth/lie statements).
    """
    from datasets import load_dataset

    kwargs: dict = {"path": "lukaemon/bbh", "name": "web_of_lies"}
            ...
            examples.append(
                {
                    ...
                    "chain_depth": _count_chain_depth(row["input"]),
                }
            )
    return examples
...
def build_chat_conversation_continual(ex: dict) -> list[dict]:
    """Pre-fill 'Answer:' as the assistant turn to anchor the completion."""
    return [
...
def accuracy_by_chain_depth(rows: list[dict]) -> dict[int, dict]:
    depths = sorted({int(r["chain_depth"]) for r in rows})
    out: dict[int, dict] = {}
    for d in depths:
        sub = [r for r in rows if int(r["chain_depth"]) == d]
        n = len(sub)
        out[d] = {
            "n": n,
            "accuracy": sum(1 for r in sub if r["correct"]) / n if n else 0.0,
