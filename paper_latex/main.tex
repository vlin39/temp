...
\paragraph{Entity Tracking Task} We use the \emph{Boxes} task from~\cite{kim-schuster-2023-entity}. Each example consists of a short narrative describing an initial inventory of $N$ boxes, each containing at most $M$ objects, with an expected number of $K$ objects, followed by a sequence of operations drawn from $\{\texttt{move}, \texttt{remove}, \texttt{put}\}$. The model is then asked to predict the contents of a single queried box after all applied operations.
